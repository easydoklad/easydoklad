          ...
          <div class="flex flex-row items-center gap-3">
            <TabsList>
              <div v-for="entry in state.value.localizedValue" class="relative">
                <TabsTrigger
                  :value="entry.locale"
                  :class="cn({ 'pr-8': disabled !== true && state.value.localizedValue.length > 1 && !isRequired(entry.locale) })"
                >
                  <Tooltip>
                    <TooltipTrigger type="button"><Flag class="size-5" :locale="entry.locale" /></TooltipTrigger>
      ...
      <div
        ...
        <DropdownMenu v-if="state.value.localized && availableLocales.length > 0" v-model:open="open">
          <DropdownMenuTrigger as-child>
            <Button size="sm" variant="link" plain class="text-xs p-0">
              Pridať preklad
            </Button>
          </DropdownMenuTrigger>
              ...
              <CommandList>
                ...
                <CommandGroup>
                  <CommandItem
                    ...
                    :value="locale.name"
                  ><Flag :locale="locale.code" /> {{ locale.name }}</CommandItem>
                </CommandGroup>
              </CommandList>
            </Command>
          </DropdownMenuContent>
